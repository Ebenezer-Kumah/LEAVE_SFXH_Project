...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['apply_leave'])) {
    try {
        ...
        // Get employee name
        $emp_query = "SELECT name FROM Users WHERE user_id = :emp_id";
        $emp_stmt = $db->prepare($emp_query);
        $emp_stmt->bindParam(':emp_id', $_SESSION['user_id']);
        $emp_stmt->execute();
        $employee_name = $emp_stmt->fetch(PDO::FETCH_ASSOC)['name'] ?? 'Employee';
        ...
        if ($duration > $available_days) {
            $error = "Insufficient leave balance. You have {$available_days} days available but requested {$duration} days.";
        } else {
            // Get manager ID
            $query = "SELECT d.manager_id 
                      FROM Users u 
            ...
            $query = "INSERT INTO Leave_Requests (employee_id, leave_type, start_date, end_date, reason, manager_id) 
                      VALUES (:emp_id, :leave_type, :start_date, :end_date, :reason, :manager_id)";
            $stmt = $db->prepare($query);
            $stmt->bindParam(':emp_id', $_SESSION['user_id']);
            ...
            if ($stmt->execute()) {
                ...
                // Notify employee
                $emp_message = "Your leave request for {$leave_type} from {$start_formatted} to {$end_formatted} has been submitted and is pending approval.";
                insertNotification($db, $_SESSION['user_id'], $emp_message);
                
                // Notify manager if exists
                if ($manager_id) {
                    $mgr_message = "New leave request from {$employee_name} for {$leave_type} from {$start_formatted} to {$end_formatted}.";
                    insertNotification($db, $manager_id, $mgr_message);
                }
                
                $message = 'Leave application submitted successfully! It is now pending approval.';
                ...
                <div class="tips-list">
                    ...
                    <div class="tip-item">
                        ...
                        <div class="tip-content">
                            ...
                            <p>Include clear reasons for your leave to help managers make informed decisions.</p>
                        </div>
                    </div>
                    ...
                    <div class="tip-item">
                        ...
                        <div class="tip-content">
                            <h4>Check Balance</h4>
                            <p>Always verify your available leave balance before submitting requests.</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <form method="POST" action="" id="leaveForm">
            <div class="modal-body">
                <div class="form-row">
                        ...
                        <select id="leave_type" name="leave_type" required onchange="updateLeaveBalance()">
                            ...
                            <?php foreach ($leave_policies as $policy): 
                                ...
                                foreach ($leave_balances as $b) {
                                    if ($b['leave_type'] === $policy['leave_type']) {
                                        $balance = $b['remaining_days'];
                                        break;
                                    }
                                }
                            ?>
                                <option value="<?php echo $policy['leave_type']; ?>" data-balance="<?php echo $balance; ?>">
                                    <?php echo $policy['leave_type']; ?> (<?php echo $balance; ?> days available)
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        <label for="leave_balance">Available Balance</label>
                        <input type="text" id="leave_balance" value="—" disabled>
                    </div>
                </div>
                ...
                <div class="form-note">
                    ...
                    Your leave request will be sent to your manager for approval. You'll be notified once it's processed.
                </div>
            </div>
